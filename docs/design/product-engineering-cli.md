...
**Honest limit.** An unjailed agent runs as you and can delete the control dir; the heartbeat closes that window to about two seconds for other processes, and the owner never trusts disk. Only a hardened backend keeps the folder out of the agent's reach.
