...
1. **Before spawn** (under the lock, just before the P snapshot):
   ...
   - The tree is walked with `lstat` only, never following links, pruning `.git` and `.legion-cli/{cache,index,sandbox,worktrees}` by name (`.legion-cli` itself is always walked, even though `init` gitignores it, so another run's cache stays observable), recording `{size, mtimeMs, kind, ino, ctimeMs, nlink, mode}` keyed by NFC and case-folded on win32/darwin (the on-disk spelling is kept alongside). **Nothing is hashed here except what is backed up or over the cap.**
