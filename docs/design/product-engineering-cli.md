...
`.legion-cli/index/engine.lock` (gitignored). CLI and dashboard POSTs take `engine.lock` via `LegionEngine` (`#mutate` / `withLock`) before any state write or spawn. GET/SSE do not. Timeout 30s → refuse “another legion-cli is running.” Within one process (the dashboard shares one engine), callers queue on an in-process mutex first; only a nested `withLock` in the same async chain re-enters. The lock file records `{pid, pidStartedAt, acquiredAt}`. It is stolen only when it is empty or unparseable and more than 10 s old, when its PID is dead, or when its PID is alive with a different start time (PID reuse). A lock held by a live process is never stolen by age; if its start time can't be read, the refusal names the PID and how to check it and delete the file.

Every source-of-truth write is atomic: a temp file in the same directory, fsync, rename (retried for up to 2 s on Windows sharing violations), refusing a symlink or junction anywhere between the project root and the target. Readers retry a failed parse briefly (5 × 20 ms, plus ENOENT and sharing violations on Windows). Task, packet and assumption ids come from file *names* (maximum plus one, created with O_EXCL), so a corrupt file's id is never reused. A task file that is not a valid task is listed, never dropped: `review`, `qa`, `ship`, `execute` and `next` refuse by file name until it is fixed, and the dashboard and MCP show it. Compaction is a **shipped extra**: lock-held; skips a `done` task when a **same-spec** sibling is `in_progress` (as §3.6).
