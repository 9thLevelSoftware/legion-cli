...
- **Incident (per run only).** Any other P change is an incident: execute leaves the task `blocked` with `incident: true`, review records `lastReview: FAIL`, other verbs refuse this invocation. A failed restore always refuses and lists what was not restored. `protected_restored` and `quarantine_created` (with the manifest's sha256) audit events are recorded after the restore. Nothing is persisted globally; **`task retry` (PR 6) is the supported exit** — until it lands, a blocked task is reopened with `legion-cli task amend` plus an explicit status edit, and the incident message says so.
...
- **Interrupted runs.** Ctrl-C or a killed engine means the compare never runs: the agent's `.legion-cli` writes stay until crash replay (`task retry`, PR 6) verifies them. §5.2's byte-protection guarantee covers runs that reach their finish.
- **The guarantee is "as of the finish".** P is compared, restored and then verified once more at the finish; it is not a live guard. The finish aborts the agent handle, but a *detached* process the agent left behind (`setsid sh -c 'sleep 30; …'`, `start /b`) survives a normal exit and can write into `.legion-cli/` afterwards, with no incident, quarantine or audit line — the compare has already happened and the freeze is already lifted. Killing such descendants for real needs a POSIX group kill plus a Windows job object, which is PR 6's interrupt work; until then the honest statement is that P protects the finish instant, not the minutes after it.
...
- an **unreadable or invalid** marker counts as live until it is older than the maximum timeout plus 10 minutes (a future mtime is clamped to now, so a planted record can never wedge the project forever);
