...
stateDiagram-v2
    ...
    Abandoned --> IntentDraft: legion-cli spec new
```
...
| `blocked` | Contract violation, failed `verificationCommands`, a non-zero agent exit, an unreadable `extra.json`, an interrupted run that was replayed, or an open blocking assumption. **Recoverable**: `legion-cli task retry <id>` moves it back to `ready`. Infrastructure failures (sandbox, adapter launch, preconditions) never get here — they leave the task `ready`. |
...
| `legion-cli execute` in `advisory` | Yes | `legion-cli control-mode guarded` |
...
| `legion-cli execute` when the sandbox, the adapter launch or a precondition fails | Yes — the task is **left/returned to `ready`**, never blocked | `legion-cli doctor` |
| Agent exits non-zero (or with no exit code and no timeout) | execute → `blocked` with `agent exited <n>` plus the last 20 stderr lines; review → `FAIL`; other spawns refuse | `legion-cli task retry <id>` |
| A run's `extra.json` that is unreadable or not a list of tickets | Task `blocked` (a **missing** file just means "no extras") | `legion-cli task retry <id>` |
| `legion-cli task retry <id>` when the task is not `blocked`, not in the active slice, or has no file contract | Yes | `legion-cli status --blockers` / `legion-cli task amend` |
| `legion-cli doctor --clear-stale-run` while the recorded engine or agent process is alive | Yes | `legion-cli status` |
...
| `control_mode: autonomous` | Config rejected; CLI `control-mode autonomous` refuses | `legion-cli control-mode` |
...
-- store.rebuild() (ingest also inserts); CLI `index rebuild` is shipped:
...
`legion-cli execute` runs **one** task (predictable for non-coders). `legion-cli execute --until-blocked` loops until no `ready` task remains; a task that **blocks** does not stop the loop — independent work carries on and the run still exits non-zero — but an **incident** does, because the tree was disturbed and the quarantine needs looking at. Either way, product phase stays `executing` until the slice is terminal (every task `done` or `blocked`).
...
| `grok` | `grok` (override `adapter.grok.binary`) | frozen `["-p", "{{pointer}}"]`; extra flags must keep `-p` + `{{pointer}}` | Allowlist (basename `grok`) | Staging dir in prompt | Process group |
| `openai` | `codex` — **alias id** for the Codex CLI (override `adapter.openai.binary`) | frozen `["exec", "{{pointer}}"]`; extra flags must keep `exec` + `{{pointer}}` | Allowlist (basename `codex`) | Staging dir in prompt | Process group |
...
- **Incident (per run only).** Any other P change is an incident: execute leaves the task `blocked` with `incident: true`, review records `lastReview: FAIL`, other verbs refuse this invocation. A failed restore always refuses and lists what was not restored. `protected_restored` and `quarantine_created` (with the manifest's sha256) audit events are recorded after the restore. Nothing is persisted globally; **`legion-cli task retry <id>` is the supported exit**: it moves the task `blocked → ready`, audits `task_retry`, clears `STATE.quarantinedCommits` (the commits themselves stay at `refs/legion-quarantine/<runId>`) and invalidates `lastReview`, so a gate needs a fresh review afterwards.
- **P cannot tell who wrote.** The freeze stops the CLI, not your editor: any change to `.legion-cli/` made while a run is live — **including your own edits** — is quarantined and reverted, and reported as part of the incident. Finish the run (or wait for the refusal) before editing engine state by hand.
- **Over the inline cap.** A protected file larger than 4 MiB is snapshotted by size and hash only. When it changes, a copy of the agent's version goes to quarantine, the agent's version **stays in place** (moving it would delete data), and the run refuses with "over 4 MiB … restore the file yourself from git".
- **Interrupted runs.** Ctrl-C is handled: the first SIGINT/SIGTERM kills the agent (the POSIX process group, or the Windows process tree) and then runs the spawn's normal finish — restore P, revert the tree, lift the freeze — before exiting 130; a second signal exits immediately. A *killed engine* cannot do that, so the next verb **replays** the run under the lock before it writes anything: it kills the recorded agent tree (only when the PID's recorded start time still matches, so a reused PID is never killed), restores P from the control-dir snapshot and the tree from the manifest and backups (quarantine first), destroys the leftover jail, fails the last review, records an incident and blocks the task with "an interrupted run was reverted; legion-cli task retry <id>". The replay never trusts the persisted snapshot: it fails the review and blocks the task whatever those bytes say, so a forged `lastReview: PASS` buys nothing. Replay is narrower than the live revert — it puts back the paths the manifest recorded a source for; a file the agent *created* is left in place and named in the quarantine folder instead.
- **The guarantee is "as of the finish".** P is compared, restored and then verified once more at the finish; it is not a live guard. The finish aborts the agent handle, but a *detached* process the agent left behind (`setsid sh -c 'sleep 30; …'`, `start /b`) survives a normal exit and can write into `.legion-cli/` afterwards, with no incident, quarantine or audit line — the compare has already happened and the freeze is already lifted. The finish, the interrupt handler and the crash replay all kill the process *group* (POSIX) or the process *tree* (`taskkill /T`, Windows), which covers everything that stayed in the group. A process that deliberately left it — `setsid`, a Windows detached process — still survives, because catching that needs a Windows job object the engine does not create. So the honest statement stands: P protects the finish instant, not the minutes after it.
- **Not in P.** `HEAD`, `refs/**` and `packed-refs` stay outside P on purpose: restoring them would silently undo an agent commit, which is a *warning* here (`HEAD_MOVED_WARNING`) and becomes a recorded incident in PR 5. Submodule control files (`modules/<name>/{config,config.worktree,hooks}`) *are* protected, and read calls also set `GIT_CONFIG_NOSYSTEM=1` and an empty `GIT_CONFIG_GLOBAL`, so no config outside the repository can add a filter or diff driver. Two keys people legitimately set globally are read back from the system and global scopes only (never from the repository config, which an agent can write) and re-injected with `-c`: `safe.directory` (container and shared checkouts) and, on Windows, `core.longpaths`.
- **Quarantine (R-17, R-40).** `<userStateDir>/quarantine/<projectHash>/<runId>-<random>/` (`%LOCALAPPDATA%\legion-cli`, `~/Library/Caches/legion-cli` or `${XDG_STATE_HOME:-~/.local/state}/legion-cli`, or `LEGION_CLI_STATE_DIR` when it is set to an absolute path), created at finish with an exclusive mkdir, owner-only, refused if any level is a link, with a `MANIFEST.json` (`path`, `reason`, `restoredFrom`, `sha256`, `at`). Moves never overwrite (hard link + unlink; across volumes, exclusive copy, verify the sha, then delete the source). It is outside the project, so it is never committed and project cleanup (`git clean -fdX`) can't reach it. The engine never deletes it; `doctor` lists retained quarantines and flags a missing manifest or one that doesn't match its audited hash.

**Freeze (KD-2).** While any agent spawn is live, every engine write is refused: "an agent run (<skill> <runId>) is in progress; wait for it to finish (legion-cli status)". The check runs first in every locked verb, before crash recovery, and again under the lock; chat session writes and the skill / design-system installers check it before *and* under the lock too. Reads (status, show, brief, search, MCP, dashboard GET) keep working. `legion-cli status` and `doctor` print the run that holds the freeze — skill, run id, engine PID, start time, and when a stale record ages out — so it is never invisible.

The owning process knows its spawn from memory and its finish path is exempt through an in-memory token. Other processes read the run's control record in `<userStateDir>/control/<projectHash>/<runId>/` (`resume.json`, `live.json`), never in the agent-writable run cache. `LEGION_CLI_STATE_DIR` (absolute) overrides that location; the test suites use it to stay out of the real profile. Liveness for another process:

- a **valid** marker (`startedAt` not in the future, `timeoutMs` ≤ 20 min) whose engine PID is alive and not reused is live;
- an **unreadable or invalid** marker counts as live until it is older than the maximum timeout plus 5 minutes (a future mtime is clamped to now, so a planted record can never wedge the project forever);
- a **missing** marker counts as live while the run's recorded engine or agent process is still alive, and otherwise until the run record ages out the same way — deleting the record is not an escape hatch, and the owner re-asserts its marker every 2 s while the run lasts;
- a **dead engine whose agent process is still running** keeps the freeze until that run's own `startedAt + timeoutMs + grace` passes (PR 6's replay tightens this);
- a record proven dead is removed by the reader, and a clean finish deletes the run's control dir, so the check stays proportional to in-flight runs.

**Honest limit.** An unjailed agent runs as you and can delete the control dir. The heartbeat limits a *single* deletion to about two seconds of unfrozen time for other processes, but an agent that deletes the record in a loop keeps that window open continuously — the owner still never trusts disk, and its finish still restores P. Only a hardened backend keeps the folder out of the agent's reach.

**Write ordering (KD-15).** The engine never writes into P between the snapshot and the restore. Spawn-related writes (the task's `in_progress` transition, `STATE.md`, execute's `sandbox_start` / `sandbox_degraded` event) happen before the snapshot. Audit events this process raises during the spawn are buffered in memory; audit events from other processes (freeze refusals, lock timeouts, dashboard refusals) go to `<controlDir>/deferred-audit.jsonl`. The owning finish appends both after the restore, keeping their timestamps.

Deferred content is **not trusted**: the control dir is reachable by an unjailed agent, so only `refuse` events are drained, each one is re-stamped `actor: "deferred"` with `data.deferred: true` and its control dir, a future or unparseable timestamp is replaced by the drain time, and the file is capped (256 KiB, 500 events). `doctor`'s quarantine-integrity check ignores deferred events, so a forged `quarantine_created` can never make a tampered quarantine read as intact.

Write paths under `.legion-cli/` outside `#mutate`, and why each is safe:

| Path | Writer | Why it cannot corrupt P |
|---|---|---|
| `audit/**` | `#audit` on every verb | in P; buffered during the window, appended after the restore |
| `serve.json` | `legion-cli serve` claim/release | excluded from P (runtime state, token hash only) |
| `index/**` (sqlite, `engine.lock`) | store rebuild, lock | excluded from P; rebuilt from the files, and the finish rebuilds it after a restore |
| `chat/*.json` | `saveChatSession`, `resumeOrCreateChatSession` | in P; freeze-checked before and under the lock |
| `skills/**`, `design/**` | `skills install`, `design-system install` | in P; freeze-checked before and under the lock (`installWithLock`) |
| `cache/runs/<id>/**`, `cache/skills/<id>/**` | staging, prompts, logs, `extra.json` | excluded from P; only *this run's* cache is exempt from the revert |
...
1. **Before spawn** (under the lock, just before the P snapshot):
   ...
   - One `git ls-files -z` gives the **authoritative tracked set**. "Tracked-clean" is never inferred by elimination: a submodule and a nested checkout are reported by neither `status` nor `ls-files -i`, so inferring would leave them with no backup and no git source.
   - One `git ls-files -s -z` records the index, so a change that only touches the index — `git add`, `git rm --cached` — is detected even though nothing on disk moved.
   - One `git for-each-ref` records every ref, so a deleted branch, a force-moved one or a dropped stash is noticed even when `HEAD` never moved.
   - The tree is walked with `lstat` only, never following links, pruning `.git`, `.legion-cli/{index,sandbox,worktrees}` and **this run's own** `cache/runs/<runId>` and `cache/skills/<runId>`. `.legion-cli` itself is always walked even though `init` gitignores it, so another run's cache stays observable (PR 4's R-33); a change there is **reported and left in place**, never restored and never an incident, because KD-2 means the engine trusts nothing in it. The walk records `{size, mtimeMs, kind, ino, ctimeMs, nlink, mode}` keyed by NFC and case-folded on win32/darwin (the on-disk spelling is kept alongside). **Nothing is hashed here except what is backed up or over the cap.**
   - **Backups** go to `<controlDir>/pre/` (owner-only) for dirty and untracked non-ignored files, for **every pre-existing ignored file outside ignored directories**, and for secret-like names at the top level of an ignored directory, secret-like names first so the cap never squeezes them out. Up to 4 MiB each and 256 MiB per run they are **copied** and hashed; above that they are **hardlinked**, which is O(1), costs no space and survives `rm`, `git clean -fdx` and rename-over — but not an in-place truncating write, which changes the shared inode's stat and is therefore detected rather than silently trusted. Each entry records `restoreFrom: "git" | "backup" | "none"`, and a file the pass could not protect at all is hashed instead, so merely touching it cannot block the run.
2. **After `wait()`** (after the P restore): re-walk (pruning the **pre-spawn** ignored directories, so a directory the agent newly ignored is still enumerated) and collect candidates — any path whose stat, kind, identity (`ino`, `ctimeMs`, `nlink`, `mode`) or on-disk spelling changed, plus anything added or removed. Unicode normalization is not part of a spelling, so NFC and NFD forms of one name are one path. **`.gitattributes` and `.gitignore` candidates are quarantined and restored first**: git honours them even when brand new and untracked, and they decide both what `hash-object` produces and what counts as ignored. Each remaining candidate is then **confirmed by content**:
...
5. **Agent commits and ref movement (R-20, R-32).** If `HEAD`, the checked-out branch, or **any other ref** differs from the pre-spawn snapshot — a deleted branch, a force-move that is not a fast-forward, a dropped stash, a deleted tag — the run is an **incident** (task blocked). The commits (`git rev-list <preSpawnRef>..HEAD`) are appended to `STATE.quarantinedCommits` — that field is in P, so a later agent cannot erase it — pinned at `refs/legion-quarantine/<runId>`, and **bundled to `<controlDir>/agent-commits.bundle`, outside `.git`**, so a later `gc --prune=now` cannot erase the evidence. A failed `update-ref` is reported rather than claimed. The recovery step is printed with what it costs: `git reset <preSpawnRef>` (or `git checkout <original branch>`) discards every commit made during the run, including any the human made in another terminal. The engine never moves your refs itself, and the working tree is already reverted. Recording happens on the one finish path every spawn goes through, so it is not per-verb.
6. For each confirmed change **not** in `allowed` (or in implicit forbidden), in its own try/catch, quarantines of a case-folded key before restores of the same key (R-25):
   - **Check the restore source first.** A missing blob, an unreadable backup or a backup whose sha no longer matches means the agent's version is **left exactly where it is** and the run is an incident — the path is never emptied by a move whose restore then fails.
   - `lstat` every ancestor; a link or junction ancestor is quarantined as a link (`unlink`, never recursed) and replaced by a real directory (A-025). A path this pass already restored is never displaced again.
   - **Quarantine second** — a non-overwriting move through the EPERM/EBUSY retry helper. If the quarantine fails, the path is **not** restored and the run is an incident. Only a real directory is moved recursively, and then its children are not processed again; a directory whose *spelling* changed is renamed in place instead, so its children stay individual candidates.
   - **Then restore.** From git: batched `git cat-file --batch --filters` (per-file fallback) with `GIT_NO_REPLACE_OBJECTS=1`; when the filtered form's length disagrees with the recorded pre-run size (an LF file in an autocrlf repo), the raw blob is used instead, so the restore is byte-identical to what was on disk rather than to git's canonical checkout form. From a backup: the sha256 must match. **All git-sourced restores are then re-hashed in one batched `hash-object` call** and each must equal the `preSpawnRef` blob (R-18). On any mismatch the run is an incident and the quarantined copy is kept.
   - **The index is reset** for every out-of-contract path whose `ls-files -s` entry differs from the pre-spawn one (`git reset <preSpawnRef> -- <paths>`), so content the agent staged cannot survive into `ship`.
   - `read` git calls re-inject `core.autocrlf`, `core.eol` and `filter.*` from the **system and global** scopes only — never from the repository config, which an agent can write. Without them an LFS repo's "verified" restore writes the 130-byte pointer over the asset, and a Git-for-Windows checkout's CRLF bytes can never be confirmed unchanged.
   - Nothing is ever `rm`'d, and `git clean -fd` / `git reset --hard` are never run.
7. **Ignored-path policy (KD-16, R-7).** Decided by the **pre-spawn** rules — the ignore files are restored before the query, so an agent cannot gitignore its own out-of-contract file on the way out and be exempted.
   ...
   - A modified or deleted pre-existing ignored file **outside** ignored directories is quarantined and restored from its backup. It is reported separately from the contract extras, so the agent's own `pnpm build` touching a `.tsbuildinfo` does not block the task or file a scope ticket.
   - One that could not be backed up at all produces a named `LOST:` warning and the agent's version is left in place — except a secret-like name, which is quarantined and made an incident rather than left holding agent content.
   - Changes deeper than the top level inside pre-existing ignored directories are **not tracked**. A documented limit of the stat-first design, as is an edit that forges size, mtime, inode and change time together outside P.
   - A hardlinked backup shares its inode with the project file, so it survives `rm`, `git clean` and rename-over but **not** an in-place rewrite. That case is detected from the recorded stat of the shared inode — the file is reported `LOST:` and the agent's version is left in place — never confirmed as unchanged.
   - The POSIX mode is part of a path's identity: a bare `chmod` on an out-of-contract file is reverted along with its content, a restored `0755` script stays executable, and a recreated directory gets its pre-run mode. A mode change on a file with no restore source is reported rather than reverted. On win32 the mode carries only the read-only bit, which is compared like any other stat field but not reapplied.
8. **Git failures.** If git cannot classify the tree **before** the spawn (`ls-files`, `ls-files -i` or `status` failing — a corrupt index, a locked `.git`), the spawn is refused outright: nothing is walked, nothing is copied, and the user is told Legion cannot protect their files. Afterwards git is only a restore source: if it errors then, the backup restores still complete and the run becomes an incident (F-044).
...
10. **Backup retention (R-8).** A finish with no incident deletes the whole run control dir. An incident run keeps it — the quarantine manifest references its backups — as does a crash, for replay. Those backups are plaintext copies of files git cannot reproduce, `.env` among them, so any that is older than 30 days is dropped at the start of the next run, and `doctor` reports the retained size, the path and that caveat. It counts only the runs that actually kept backups.
11. **Budget (KD-16).** Measured on the dev box with `node_modules` installed: pre-spawn snapshot ≤ 2 s and post-spawn diff ≤ 2 s; a CI guard builds a synthetic 20,000-file tree (15,000 ignored, 200 changed) and asserts snapshot + diff + restore ≤ 20 s, with an upper bound on the walked entry count so a walk that descends into the ignored tree fails loudly. Because it is a wall-clock guard it runs in its own serial step (`test/budget/`), not alongside thirty other test files.
...
`legion-cli spec new` (from `shipped` **or `abandoned`**) starts `intent_draft` for the next increment; previous spec `status: superseded`. Abandoning is therefore not a dead end: the receipt stays, and the project moves on.
...
`assume list` / `assume answer` / `index rebuild` / `control-mode` are **shipped CLI** (Always-on / Board extras). Docs follow shipped code.
...
| `legion-cli task retry <id>` | Move a blocked task back to ready (the one exit from an incident) | — |
...
Not registered yet. Do **not** call these “later.” Unknown-command is honest; “later” is not.

| Command | Notes |
| --- | --- |
| *(none remaining in this series)* | `control-mode` and verified vendor argv have landed |
...
export interface ExtraAdapterConfig {
  binary?: string;
  args?: string[]; // prefix-compatible with frozen vendor argv; must include {{pointer}} or detect() fails
}
...
export interface LegionConfig {  // config.yaml
  ...
  adapter: { // top-level + nested extra .strict() — apiKey / apiBase / model / provider fail parse
...
- **Depends on:** PR-05
- **Description:** `claude` + `generic` + `fake` spawnable. No product-default adapter. `fake` writes expected artifacts for tests. Conformance: detect matrix + spawn pointer prompt. Extras spawn with verified vendor argv (KD5); they are not detect-only.
...
- **Depends on:** PR-11
- **Description:** `legion-cli verify` is optional notes (not a ship gate). Spec `legion-cli review` when the slice is terminal. After spawn, if any new task id exists **or an existing `TSK-*.md` changed** → `lastReview: FAIL` and stay `executing`; PASS only when zero new tasks and existing task files are byte-identical. `legion-cli qa` then requires a later PASS review. Review spawn SkillContract `.legion-cli/qa/**` + `.legion-cli/tasks/**` (new fix tasks only; in-place rewrite is not a PASS license). Historical: “No packets” — packets later shipped as an extra (PR-21; rev 10).
