...
export const HINT = {
  ...
  taskUnblock: "legion-cli task amend --unblock",
  taskRecover: "legion-cli task amend --recover",
  undo: "legion-cli undo",
  blockers: "legion-cli status --blockers",
  advisory: "legion-cli control-mode guarded",
  execute: "legion-cli next / legion-cli execute",
  allowNoSandbox: "legion-cli execute --allow-no-sandbox",
  ...
  brownfieldState: (id = "<id>") => `legion-cli brownfield state ${id}`,
  brownfieldRoster: (id = "<id>") => `legion-cli brownfield roster ${id} (then write analysis/<specialist>.md)`,
  brownfieldDesign: (id = "<id>") => `legion-cli brownfield pr-plan ${id} (design.md needs a '## PR Plan' section)`,
  brownfieldPrPlan: (id = "<id>") => `legion-cli brownfield pr-plan ${id}`,
  brownfieldDag: (id = "<id>") => `legion-cli brownfield dag ${id}`,
  brownfieldPatterns: "legion-cli brownfield patterns --add <lesson>",
