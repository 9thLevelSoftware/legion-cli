...
const V0_SURFACE = [
  ...
  ["legion-cli plan", "Break into tasks I can see on the board", "--adapter"],
  ["legion-cli next", "What is unblocked?", ""],
  ["legion-cli execute [id]", "Do the next ready task", "--fix, --until-blocked, --adapter"],
  ["legion-cli ticket create", "Park extra work", "--parent, --title, --from-agent, --adapter, --route"],
  ["legion-cli task amend", "Human changes a file contract", "--allow-deps, --adapter, --route, --clear-adapter"],
  ["legion-cli verify [id]", "Optional walkthrough notes (not a ship gate)", "--adapter"],
  ["legion-cli review", "Spec-level review; fix tasks mean FAIL and re-review", "--adapter"],
  ...
  ["legion-cli fix <bug>", "Test first (must stay RED), then fix", "--adapter"],
