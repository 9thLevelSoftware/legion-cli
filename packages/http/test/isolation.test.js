...
test("http package does not import sandbox, agents, core, or wiki", async () => {
  const pkg = JSON.parse(await readFile(join(pkgRoot, "package.json"), "utf8"));
  assert.equal(pkg.name, "@9thlevelsoftware/legion-cli-http");
  const deps = {
    ...pkg.dependencies,
    ...pkg.devDependencies,
  ...
  assert.equal(deps["@9thlevelsoftware/legion-cli-sandbox"], undefined);
...
test("source does not import sandbox", async () => {
  ...
  for (const file of files) {
    ...
    for (const line of text.split(/\r?\n/)) {
      ...
      assert.doesNotMatch(line, /legion-cli-sandbox/, file);
      assert.doesNotMatch(line, /legion-cli-agents/, file);
