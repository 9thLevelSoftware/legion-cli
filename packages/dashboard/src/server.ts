...
function setSecurityHeaders(res: ServerResponse, origin: string | undefined): void {
...
export async function startDashboard(opts: DashboardOptions): Promise<DashboardHandle> {
  ...
  const handle = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
    ...
    if (!allowed) {
      setSecurityHeaders(res, undefined);
      res.statusCode = 403;
      res.setHeader("Content-Type", "text/plain; charset=utf-8");
    ...
    if (pathname === "/") {
      send(res, 200, renderKanban(snapshot), "text/html; charset=utf-8", headOnly, cors);
      return;
    }
    if (pathname === "/spec") {
      send(res, 200, renderSpec(snapshot), "text/html; charset=utf-8", headOnly, cors);
      return;
    }
    if (pathname === "/graph") {
      send(res, 200, renderGraph(snapshot), "text/html; charset=utf-8", headOnly, cors);
      return;
    }
    if (pathname === "/audit") {
      send(res, 200, renderAudit(snapshot), "text/html; charset=utf-8", headOnly, cors);
      return;
    }
    ...
    if (pathname === "/wiki" || pathname === "/wiki/") {
      ...
      send(res, 200, renderWikiIndex(pages), "text/html; charset=utf-8", headOnly, cors);
      return;
    }
    if (pathname.startsWith("/wiki/")) {
      ...
      try {
        ...
        send(res, 200, renderWikiPage(shown, links), "text/html; charset=utf-8", headOnly, cors);
      } catch (err) {
