...
- Libraries (shipped): `@9thlevelsoftware/legion-cli-{schema,core,persist,wiki,graph,agents,qa,dashboard,design-system,sandbox,map,http}`.
...
- Publish guard: root `package.json` `legionPublishAllowlist` is the workspace publish allowlist. Do not publish a workspace package that is not on that list. `@9thlevelsoftware/legion-cli-http` is on the list (public, `workspace:*` runtime dependency of shipped packages).
- Workspace packages on the allowlist are public under `@9thlevelsoftware`.
