...
- PATH bins: `legion-cli` and `legion` (same `dist/bin.js`). Installer flags (`--claude` / `--cursor` / `--windsurf` / `--codex` / `--gemini` / `--antigravity` / `install` / `plugin`) refuse exit 2; plugin installer is `npx @9thlevelsoftware/legion --claude` (bin `legion-plugins`).
