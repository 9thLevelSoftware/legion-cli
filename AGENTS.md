...
- CLI skeleton: `init`, `status` (default), `doctor`, `ingest`, `wiki trust`, `search`, `show`, `brief`, `help`. `init` requires `--adapter` (or a TTY prompt). `--mode brownfield` is shipped (not greenfield-only).
